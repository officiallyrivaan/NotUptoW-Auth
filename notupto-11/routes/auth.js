...
router.post('/signup', authLimiter, async (req, res) => {
  try {
    ...
    // Check if this IP already accepted T&C as a guest — carry it over
    const ip = req.headers['x-forwarded-for']?.split(',')[0].trim() || req.socket.remoteAddress || '';
    const guestRecord = ip ? await GuestAcceptance.findOne({ ip }) : null;
    const termsAccepted = !!guestRecord;

    const user = await User.create({ username: clean, passwordHash, role, termsAccepted });

    // Clean up the guest record — it's now merged into the user account
    if (guestRecord) await GuestAcceptance.deleteOne({ ip });
...
router.get('/permissions/users', requireAuth, requireOwner, async (req, res) => {
  try {
    const users = await User.find({}, 'username role termsAccepted createdAt').sort({ createdAt: 1 });
